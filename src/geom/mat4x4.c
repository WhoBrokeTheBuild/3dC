...
Mat4x4_Equals(Mat4x4 * matA, Mat4x4 * matB)
{
    ...
    return (Vec4_Equals(&(matA->vecs[0]), &(matB->vecs[0])) && Vec4_Equals(&(matA->vecs[1]), &(matB->vecs[1])) && Vec4_Equals(&(matA->vecs[2]), &(matB->vecs[2])) && Vec4_Equals(&(matA->vecs[3]), &(matB->vecs[3])));
}
